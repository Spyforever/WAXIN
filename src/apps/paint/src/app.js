...
// update_from_url_params(); // Moved to the end of the file to ensure $toolbox and $colorbox are defined
...
const $app = $(E("div")).addClass("jspaint").appendTo(window.paintAppContainer || "body");
window.$app = $app;
...
if ($news_indicator.text().includes("Bubblegum")) {
	...
	const make_bubble = () => {
		const $bubble = $(E("img")).attr({
			...
			left: 0,
			zIndex: 10,
		}).appendTo(window.paintAppContainer || "body");
