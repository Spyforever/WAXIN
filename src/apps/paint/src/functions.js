...
function show_about_paint() {
	...
	if (is_pride_month) {
		$("#about-paint-icon").attr("src", "/win98-web/apps/paint/images/icons/gay-es-paint-128x128.png");
	}
...
function paste(img_or_canvas) {
	...
	if (img_or_canvas.width > main_canvas.width || img_or_canvas.height > main_canvas.height) {
		...
		showMessageBox({
			...
			windowOptions: {
				icons: {
					16: "/win98-web/apps/paint/images/windows-16x16.png",
					32: "/win98-web/apps/paint/images/windows-32x32.png",
				},
			},
			buttons: [
				{
...
function image_stretch_and_skew() {
	...
	const $RowInput = ($table, img_src, label_with_hotkey, default_value, label_unit, min, max) => {
		...
		const $img = $(E("img")).attr({
			src: `/win98-web/apps/paint/images/transforms/${img_src}.png`,
			width: 32,
			height: 32,
		}).css({
			marginRight: "20px",
...
function read_image_file(blob, callback) {
	...
	blob.arrayBuffer().then((arrayBuffer) => {
		...
		if (detected_type_id === "bmp") {
			...
			pdfjs.GlobalWorkerOptions.workerSrc = "/win98-web/apps/paint/lib/pdf.js/build/pdf.worker.js";

			const file_bytes = new Uint8Array(arrayBuffer);

			const loadingTask = pdfjs.getDocument({
				data: file_bytes,
				cMapUrl: "/win98-web/apps/paint/lib/pdf.js/web/cmaps/",
				cMapPacked: true,
			});
